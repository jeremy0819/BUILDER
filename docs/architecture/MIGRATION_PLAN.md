...
> （tag `v0.2.0-premerge`／遠端分支 `premerge-v0.2.0`）。
...
```bash
# 在兩個舊庫各自執行
grep -rn "竹蓮\|安和\|安民\|中正\|龜山" --exclude-dir=.git .   # 兩庫都要零命中
...
```
1. pytest（黃金＋合約測試）
2. grep -rn "竹蓮|安和|安民|中正|龜山" --exclude-dir=.git . → 非零命中即 fail
