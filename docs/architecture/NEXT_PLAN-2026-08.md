...
| 站 | 狀態 | 證據 |
|---|---|---|
| N1 Decision v0.2 | ✅ 完成 | `decision.schema.v0.2.json` 凍結；三條從嚴斷言綠 |
...
> N3 完成的意義：先前寫的「一張漸層圖會讓人以為每一點都算過，而『宣告網格數＝實際
> recompute 次數』這條 Case G 斷言，沒有真實瀏覽器跑過就只是紙上規格」——
> 現在有真瀏覽器了，M8.4 可以按原定驗收標準開工。
