...
## 0-bis. 已完成（2026-09-13 實測確認，`bb1074c` 已併入 main）

| 項 | 狀態 | 實測證據 |
|---|---|---|
| §1 備份提醒 | ✅ | 四步皆有「資料僅存在此瀏覽器，清除瀏覽資料可能永久遺失」＋匯出／還原／本機診斷 |
| §3 未校準標示 | ✅ | 由僅 ① 擴及四步；導覽列標「快照判定 · 存活率未校準」 |
| §6 Gate 編號撞號 | ✅ | 僅刻意拆分的 `Gate 1` 重複；新增 `tools/check_ci_gates.py` 守衛並自測 |
| §2 階段一（程式與驗證） | ✅ | `runtime_source` 回報 same-origin／cdn-fallback，僅 404 回退；13 資源 15,050,565 bytes 全帶 sha256 |
| §5.1 可觀測性 | ✅ | `local-diagnostics.js`，50 筆上限、只記錯誤類別與時間 |
| 錯誤訊息人話化 | ✅ | 抽到 `core-runtime.js` 共用；① ④ 已不再顯示原始例外 |

**仍未完成**：§2 階段二（部署，使用者裁決暫緩）、§4.2 Workspace 面板搬移（目前僅深連結）、
§4.3 後三個散鍵、§5.2 80 戶上限無聲丟棄、§5.3 無障礙（手機未動）。
