...
四動詞（穩定合約，簽章往後只增不改）：
  ...
  deserialize(data)        JSON 字串/路徑/dict → 正規化到 schema 2.0 的 dict
...
__all__ = [
    ...
    "input_hash", "CORE_VERSION", "SCHEMA_V2", "SCHEMA_VIEWS",
]
...
def _registry():
    """建 referencing Registry，讓 v2 視圖檔的跨檔 $ref 能解析到單一權威檔。"""
    from referencing import Registry, Resource
    canon = _load(SCHEMA_V2)
    return canon, Registry().with_resource(canon["$id"], Resource.from_contents(canon))
...
def deserialize(data, to_v2: bool = True) -> dict:
    """JSON 來源（字串／檔案路徑／已解析 dict）→ dict；預設鏈式遷移到 schema 2.0。

    data 可為：dict（原樣）、以 '{' 開頭的 JSON 字串、或存在的檔案路徑字串/Path。
    to_v2=True 時對舊版（1.0/1.1）自動 migrate；已是 2.0 原樣回傳。
    """
